...
import { verificarCardsDeContato } from './bot/baileys/cardsContato.js';
    ...
    c.ev.process(async(events)=>{
        ...
        if(events['messages.upsert']) {
            ...
            if(inicializacaoCompleta) { 
                ...
                // Integrando a verificação de cards de contato e caracteres
                for (const mensagem of m.messages) {
                    await verificarCardsDeContato(c, mensagem);
                    await removerCaracteres(c, mensagem);

                    // Chamando a função handleMessage para tratar advertências
                    await handleMessage(c, mensagem);
                    
                    // Chamando a função mencionarTodos
